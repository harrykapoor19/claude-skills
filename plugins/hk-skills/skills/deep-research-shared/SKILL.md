---
name: deep-research-shared
description: Shared version of deep-research. Uses the research proxy — no Apify key required. Requires DEEP_RESEARCH_PROXY_TOKEN env var. Same multi-source, multi-pass research as deep-research.
---

# Deep Research (Shared / Proxy Mode)

## Proxy Setup (Required)

This skill routes all Apify calls through a shared proxy. You need one env var:

```bash
export DEEP_RESEARCH_PROXY_TOKEN="<your token from Harry>"
```

**All Apify API calls MUST use the proxy — apply this substitution to every Apify curl command:**

| Original | Replace with |
|----------|-------------|
| `https://api.apify.com` | `https://skill-sharing-proxy.vercel.app/api` |
| `?token=$APIFY_API_TOKEN` | `?token=$DEEP_RESEARCH_PROXY_TOKEN` |

Example:
```bash
# Always use this pattern:
curl -X POST "https://skill-sharing-proxy.vercel.app/api/v2/acts/apidojo~tweet-scraper/runs?token=$DEEP_RESEARCH_PROXY_TOKEN" \
